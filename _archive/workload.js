// workload.js — HSos Workload (Vendor view)
...
function getTaskTypeName(id) { return taskTypes.find(t => t.id === id)?.name || '—' }
function getTaskTypeRate(id) { return taskTypes.find(t => t.id === id)?.rate_usd || 0 }
function getClientName(id)   { return allClients.find(c => c.id === id)?.full_name || '—' }
...
function renderClientPicker() {
  ...
  html += allClients.map(c => {
    ...
    const pkgLabel = pkg ? `${pkg.sessions_used}/${pkg.total_sessions} sessions` : 'No package'
    return `
...
function renderTaskBreakdown() {
  ...
  document.getElementById('task-breakdown').innerHTML = Object.values(byTask).map(row => `
    <tr>
      <td>${row.name}</td>
...
function renderUnpaidSessions() {
  ...
  if (!unpaidSessions.length) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;color:var(--mu2);padding:24px">No unpaid sessions</td></tr>`
    document.getElementById('unpaid-total').textContent = ''
    return
  }
  tbody.innerHTML = unpaidSessions.map(s => {
  ...
  const total = unpaidSessions.reduce((sum, s) => sum + sessionAmount(s), 0)
  document.getElementById('unpaid-total').textContent = `${unpaidSessions.length} sessions · ${fmt(total)}`
}
...
function selectAllUnpaid() { unpaidSessions.forEach(s => selectedUnpaid.add(s.id)); renderUnpaidSessions() }
window.selectAllUnpaid = selectAllUnpaid

function unselectAllUnpaid() { selectedUnpaid.clear(); renderUnpaidSessions() }
...
async function createDraftBill() {
  ...
  try {
    ...
    btn.disabled = false; btn.textContent = 'Create draft bill from selected'
  }
}
...
function viewDraftBill() {
  ...
  const rows = sessions.map(s => `
    <tr>
      ...
      <td class="mono">${fmtHours(s.hours)}</td>
      <td style="text-align:right" class="mono">${fmt(sessionAmount(s))}</td>
      <td style="font-size:11px;color:var(--mu)">${s.notes || '—'}</td>
    </tr>`).join('')
  const overlay = document.createElement('div')
  overlay.className = 'overlay open'
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:var(--r-lg);width:620px;padding:24px;max-height:80vh;display:flex;flex-direction:column">
...
async function withdrawDraftBill() {
  if (!draftBill) return
  if (!confirm('Withdraw this draft bill? Sessions will return to unpaid.')) return
  try {
    await withdrawBillV2(draftBill.id)
    await loadVendorData()
    renderWorkTab()
    showToast('Draft bill withdrawn')
  } catch (err) { showToast('Failed to withdraw', 'warn') }
}
    ...
    return `
      <div class="block" style="margin-bottom:16px">
        <div style="padding:14px 16px;border-bottom:1px solid var(--border2);display:flex;align-items:center;justify-content:space-between">
          <div>
            <div style="font-size:13px;font-weight:600;color:var(--green-text)">✅ Paid ${formatDateShort(bill.paid_at)}</div>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelector('.tb-av')?.addEventListener('click', async () => {
    ...
    if (v) {
      ...
      selectedClientId = null
      selectedUnpaid.clear()
      draftBill = null; rejectedBill = null
      await loadVendorData()
